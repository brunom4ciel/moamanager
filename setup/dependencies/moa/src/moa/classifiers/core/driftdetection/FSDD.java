...
/**
 * Fisher Square Drift Detector (FSDD)
 * published as:
...
public class FSDD extends AbstractChangeDetector {

    private static final long serialVersionUID = -3518369648142099804L;
    ...
    private double ewr, err, ewp, erp;
    private int no, nr, np;
    private double[] factorial;
    private int maximum, i;
    private double f, m_p;
    private double p0, p, X, m_n;
    ...
    public void initialize() {
        ...
        //System.out.println("FSDD - Parameters: Window Size = " + windowSize + " | Alpha Drift = " + alphaDrift + " | Alpha Warning = " + alphaWarning + ".");
    }
    ...
    public void input(double prediction) {   // In MOA, 1.0=false, 0.0=true.
        ...
        if (no >= windowSize) {   // The same as: (no + nr) >= 2 * windowSize.
            ...
            if (wr < 5 || rr < 5 || wp < 5 || rp < 5) {
                ...
                ewr = (wr + wp) * (wr + rr) / maximum;
                err = (rr + rp) * (rr + wr) / maximum;
                ewp = (wp + wr) * (wp + rp) / maximum;
                erp = (rp + rr) * (rp + wp) / maximum;
                
                X = Math.pow(Math.abs(wr - ewr), 2) / ewr;
                X = X + Math.pow(Math.abs(rr - err), 2) / err;
                X = X + Math.pow(Math.abs(wp - ewp), 2) / ewp;
                X = X + Math.pow(Math.abs(rp - erp), 2) / erp;
                p = Statistics.chiSquaredProbability(X, 1.0);                
            }
