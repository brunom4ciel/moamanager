 ...
 *
 * Code modified by Silas Garrido (sgtcs@cin.ufpe.br) to allow the use of other detectors (-d option).
 *
...
public class LeveragingBag2 extends AbstractClassifier {
    ...
    protected ChangeDetector[] CDError;
    ...
    public void resetLearningImpl() {
        ...
        this.CDError = new ChangeDetector[this.ensemble.length];
        for (int i = 0; i < this.ensemble.length; i++) {
            this.CDError[i] = ((ChangeDetector) getPreparedClassOption(this.driftDetectionMethodOption)).copy();
        }
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        for (int i = 0; i < this.ensemble.length; i++) {
            ...
            switch (this.leveraginBagAlgorithmOption.getChosenIndex()) {
                ...
                case 1: //LeveragingBagME
                    double error = this.CDError[i].getEstimation();
            ...
            double ErrEstim = this.CDError[i].getEstimation();
            this.CDError[i].input(correctlyClassifies ? 0 : 1);
            if (this.CDError[i].getChange()) {
                if (this.CDError[i].getEstimation() > ErrEstim) {
                    Change = true;
                }
            }
        }
        if (Change) {
            ...
            int imax = -1;
            for (int i = 0; i < this.ensemble.length; i++) {
                if (max < this.CDError[i].getEstimation()) {
                    max = this.CDError[i].getEstimation();
                    imax = i;
                }
            }
            if (imax != -1) {
                ...
                this.CDError[imax] = ((ChangeDetector) getPreparedClassOption(this.driftDetectionMethodOption)).copy();
            }
        }
    }
