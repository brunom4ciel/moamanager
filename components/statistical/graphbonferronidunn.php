...
$caption = "bonferronidunn";// $application->getParameter("caption");
...
$cdmethod = $application->getParameter("cdmethod");
...
if (in_array($task, $statistical_test_array)) {
    ...
    $nemenyi_bin = Properties::getbase_directory_statistical() . "orange3/bonferronidunn.py";
    ...
    $cdmethod_rank = $cols_names1[$cdmethod-1];
	...
	$cols_names_aux = explode("\t", $cols_names);
	
	$cdmethod_tmp = $cdmethod;
	
	for($i_aux = 0; $i_aux < count($cols_names_aux); $i_aux++)
	{
	    if($cols_names_aux[$i_aux] == $cdmethod_rank)
	    {
	        $cdmethod_tmp = $i_aux;
	    }
	}
...
// rene	39";
    ...
    if (is_file($filename . ".tmp")) {
		...
		$command = "python3 " . $nemenyi_bin . "  " . $filename . ".tmp  " . $filename_img . " png " . $cdmethod_tmp . " " . ($showrank ? "showrank":""); 
// 		$command = "python3 " . $nemenyi_bin . "  " . $filename . ".tmp  " . $filename_img . " png USDD " . ($showrank ? "showrank":""); 
...
//         exit($command);
		...
		$command2 = "python3 " . $nemenyi_bin . "  " . $filename . ".tmp  " . $filename_img2 . " " . $downloadfile . " " . $cdmethod_tmp . " " . ($showrank ? "showrank":"");  
// 		$command2 = "python3 " . $nemenyi_bin . "  " . $filename . ".tmp  " . $filename_img2 . " " . $downloadfile . " USDD " . ($showrank ? "showrank":"");  
        $command2 = escapeshellcmd($command2);
        
        
//         var_dump($command);var_dump($command2);exit();
...
?>
	<a target="_blank" href="?component=statistical&controller=printpreview&attachment=false&tmpl=tmpl&filename=<?php echo urlencode($filename);?>&datasource=<?php echo urlencode(base64_encode($data_rank));?>">Print Preview</a>
									...
									<div style="float: left;width: 100%; margin-top: 5px;">
										 <label>CD Method <input type="text" value="<?php echo $cdmethod; ?>" placeholder="1"
									name="cdmethod" id="cdmethod" style="width:100px"> (Order Column Reference) Bonferroni-Dunn</label><br>
