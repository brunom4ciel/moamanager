...
Framework::include("moa.php", PATH_COMPONENTS . DIRECTORY_SEPARATOR . $application->getComponent() . DIRECTORY_SEPARATOR);
...
if ($task == "open") {
    ...
    if ($filename != null) {
        ...
        $filename = PATH_USER_WORKSPACE_STORAGE . DIRNAME_SCRIPT . DIRECTORY_SEPARATOR . $folder . 
        // .DIRECTORY_SEPARATOR
    ...
    if ($task == "continue") {
        ...
        if ($task == "run") {
            ...
            $dirProcess = Properties::getBase_directory_destine_exec() . $application->getUser() . DIRECTORY_SEPARATOR;
            ...
            if (strpos($version_software, $application->getUser()) === false && strpos($version_software, Properties::getBase_directory_moa_jar_default()) === false) {

                exit("error not version software permission");
            } else {

                if (is_file(Properties::getBase_directory_moa() . "bin" . DIRECTORY_SEPARATOR . $version_software)) {
            ...
            if ($application->getParameter("foldername") != null) {
                ...
                if ($application->getParameter("dirstorage") == null) {
                    ...
                    $foldernew = $application->getParameter("dirstorage") . DIRECTORY_SEPARATOR . $fname . // $application->getParameter("filename")
                    DIRECTORY_SEPARATOR;
                }
                ...
                while (is_dir(Properties::getBase_directory_destine($application) . $application->getUser() . DIRECTORY_SEPARATOR . $foldernew__)) {
                ...
                // criar o diretorio
                mkdir(Properties::getBase_directory_destine($application) . $application->getUser() . DIRECTORY_SEPARATOR . $foldernew, 0777, true);

                // modifica as permissoes do diretorio
                chmod(Properties::getBase_directory_destine($application) . $application->getUser() . DIRECTORY_SEPARATOR . $foldernew, 0777);

                // define o local do diretorio base
                $dirStorage = Properties::getBase_directory_destine($application) . $application->getUser() . DIRECTORY_SEPARATOR . $foldernew . DIRECTORY_SEPARATOR;
                ...
                $from_folder = Properties::getBase_directory_destine($application) . $application->getUser() . DIRECTORY_SEPARATOR . "scripts" . DIRECTORY_SEPARATOR . $application->getParameter("foldername") . DIRECTORY_SEPARATOR;

                // verifica se o diretorio existe
                if (is_dir($from_folder)) {

                    // carrega a lista de arquivos de um diterório
                    $files_list = $utils->getListElementsDirectory($from_folder, array(
                        "data"
                    ));
                    ...
                    foreach ($files_list as $file_item) {
                        ...
                        $filename = substr($file_item, strrpos($file_item, DIRECTORY_SEPARATOR), strrpos($file_item, "."));

                        mkdir($dirStorage . $filename, 0777, true);

                        chmod($dirStorage . $filename, 0777);

                        $dirStorage_script = $dirStorage . $filename . DIRECTORY_SEPARATOR;
                        ...
                        foreach ($script_list as $script_item) {
                            ...
                            $cmd = Properties::getFileJavaExec() . " -Xmx" . $moa_menory_used . $moa_memory_unit . " -cp \"" . Properties::getBase_directory_moa() . "bin" . DIRECTORY_SEPARATOR . $moafile . ":" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "*\"" . " -javaagent:" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "sizeofag-1.0.0.jar moa.DoTask \ \"" . $script_item . "\" > " . $dirProcess . $filename_script;
                        ...
                        $filename = $dirStorage_script . $filename_source . ".log";
                        ...
                        $execution_history_id = $execution_history->process_initialized($user_id, 1, $script, $process_initialized, $command, $from, $pid);

                        //
                        // ------------END START PROCESS ------
                        //

                        $parallel->pool_execute($filename, $lines_cmd, $application->getParameter("parallel_process"), $dirProcess, $dirStorage_script, $user_id);
                        ...
                        if ($execution_history_id != null) {
                            $execution_history->closed_process($execution_history_id, $process_closed);
                        }
                ...
                foreach ($list_scripts2 as $key => $item) {
                    ...
                    // if(file_exists($base_directory_destine_exec.$filename)){
                    // existe

                    // unlink($base_directory_destine_exec.$filename);
                    //
                    // }

                    $cmd = Properties::getFileJavaExec() . " -Xmx" . $moa_menory_used . $moa_memory_unit . " -cp \"" . Properties::getBase_directory_moa() . "bin" . DIRECTORY_SEPARATOR . $moafile . ":" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "*\"" . " -javaagent:" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "sizeofag-1.0.0.jar moa.DoTask \ \"" . $item . "\" > " . $dirProcess . $filename;
                    ...
                    // ."lib"
                    // .DIRECTORY_SEPARATOR
                    // ."sizeofag-1.0.0.jar moa.DoTask \ \"".$item."\" > "
                    // .$dirProcess.$filename;

                    // }else{

                    // $cmd = "java -Xmx2G -cp \""
                    // .App::$base_directory_moa_jar_default
                    // .":"
                    // .App::getBase_directory_moa()
                    // ."lib"
                    // .DIRECTORY_SEPARATOR
                ...
                $filename = $dirStorage . $application->getParameter("filename") . ".log";
                ...
                // $ddd = $jsonfile->getData();

                // $ss = $jsonfile->SerializingData($dataJson);

                // $aa = $jsonfile->DeserializingData($ss);

                // var_dump($ddd);

                // $jsonfile->load();

                // $datas = $jsonfile->getContents();

                // var_dump($datas);

                // $data = $jsonfile->getDataKeyValue("id", "0002");

                // $data["process"] = true;

                // $jsonfile->setDataKeyValue("id","0002", $data);
                ...
                $parallel->pool_execute($filename, $lines_cmd, $application->getParameter("parallel_process"), $dirProcess, $dirStorage, $user_id);
            ...
            if ($notification == "1") {
                ...
                if ($email != null) {
                    ...
                    $url_report = "http://" . $_SERVER['HTTP_HOST'] . PATH_WWW;

                    $body = "Finished: " . $application->getParameter("filename") . "<br><br>" . $url_report;
                ...
                if ($phone != null) {
                    ...
                    $plivo = new Plivo(Properties::getPlivoAuthId(), Properties::getPlivoAuthToken());

                    $result = $plivo->sendSMS("Completed Experiment - " . substr($application->getParameter("filename"), 0, 30), array(
                        $phone
                    ));
            ...
            // do{
            // $foldernew = $foldername." (".$utils->format_number($y,4).")";
            // $y++;
            // }while(is_dir(App::$base_directory_destine
            // .$application->getUser()
            // .DIRECTORY_SEPARATOR
            // .$foldernew));

            // exit("bruno=".$foldernew__);
            ...
            // $dirStorage = App::$base_directory_destine
            // .$application->getUser()
            // .DIRECTORY_SEPARATOR
            ...
            foreach ($list_scripts2 as $key => $item) {
                ...
                if (file_exists(Properties::getBase_directory_moa() . "bin" . DIRECTORY_SEPARATOR . $application->getUser() . ".jar")) {

                    $cmd = "java -Xmx2G -cp \"" . Properties::getBase_directory_moa() . "bin" . DIRECTORY_SEPARATOR . $application->getUser() . ".jar:" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "*\"" . " -javaagent:" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "sizeofag-1.0.0.jar moa.DoTask \ \"" . $item . "\" > " . $dirProcess . $filename;
                } else {

                    $cmd = "java -Xmx2G -cp \"" . Properties::getBase_directory_moa_jar_default() . ":" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "*\"" . " -javaagent:" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "sizeofag-1.0.0.jar moa.DoTask \ \"" . $item . "\" > " . $dirProcess . $filename;
                }
                ...
                if ($length_data > 0) {
                    ...
                    foreach ($data2 as $key => $element) {
                        ...
                        if (is_array($element)) {
                            ...
                            if (file_exists($dirStorage . $filename_)) {} else {
                                ...
                                $filename_script = substr($cmd, strrpos($cmd, "\" >") + 4);

                                $cmd = Properties::getFileJavaExec() . " -Xmx" . $moa_menory_used . $moa_memory_unit . " -cp \"" . Properties::getBase_directory_moa() . "bin" . DIRECTORY_SEPARATOR . $moafile . ":" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "*\"" . " -javaagent:" . Properties::getBase_directory_moa() . "lib" . DIRECTORY_SEPARATOR . "sizeofag-1.0.0.jar moa.DoTask \ \"" . $element["script"] . "\" > " . $filename_script;
	...
	<div class="container" style="width: 90%">
        ...
        if (! empty($application->getParameter("foldername"))) {
            ?>
										...
										<input type="hidden"
									value="<?php echo (empty($application->getParameter("folder"))? $application->getParameter("foldername"):$application->getParameter("folder").$application->getParameter("foldername"));?>"
									...
									id="foldername">	
										...
										<input type="hidden" value="run" name="task" id="task"> <input
									type="hidden"
        ...
        if ($application->getParameter("foldername") != null) {
            ...
            ?>
											...
											name="filename" id="filename" style="width: 100%;">
										<textarea id="data"
											style="visibility: hidden; display: none; width: 100%; height: 400px;"
											name="data"
											<?php echo ($task=="continue"?"readOnly=\"true\"":"")?>><?php echo $data?></textarea>
										<br>
            ...
            ?>
										...
										<br>
											...
											<?php }?>	
												...
												<br> Number Of Process Parallel <select
											name="parallel_process" id="parallel_process"
										...
										</select> <br> Memory Used By Process <select
											name="memory_used" id="memory_used"
											onchange="setCookieElementSelectValue(this);">
										...
										</select> <select name="memory_unit" id="memory_unit"
											onchange="setCookieElementSelectValue(this);">
										...
										<div
											style="border: 1px solid #cfcfcf; box-sizing: border-box; padding: 2px;">


											<label><input type="checkbox" name="notification"
												id="notification" value="1"
												onclick="setCookieCheckbox(this);">Notification</label> <br>
											Notification by email <input type="text" id="email"
												name="email" onchange="setCookieElementValue(this);" /> <br>
											Notification by phone <input type="text" id="phone"
												name="phone" placeholder="+5581998070481"
												onchange="setCookieElementValue(this);" /> <br>

										</div>

										<br> Version of the software<select name="version_software"
											id="version_software"
											onchange="setCookieElementSelectValue(this);">
												
												<?php

            $files_list = $utils->getListElementsDirectory1(Properties::getBase_directory_moa() . "bin/", array(
                "jar"
            ));
            ...
            ?>
												</select><br> Save result in folder <select
											name="dirstorage" id="dirstorage"
											<?php echo ($task=="continue"?"disabled":"")?>>
											<option value=""></option>
													<?php

            $files_list = $utils->getListDirectory(Properties::getBase_directory_destine($application) . $application->getUser() . DIRECTORY_SEPARATOR);

            foreach ($files_list as $key => $element) {

                // if($element["type"]=="dir"){
                if ($element == DIRNAME_SCRIPT || $element == DIRNAME_TRASH || $element == DIRNAME_BACKUP) {
                    unset($files_list[$key]);
                }
                // }
            }
        ...
        ?>
											...
											onclick="javascript: sendScripts('restart');">	
												
												<?php }else{ ?>
												
												<input type="button" id="button_send" name="button_send"
											value="Execute"
											onclick="javascript: sendScripts('run'); this.disabled=true; this.style.background = '#ffffff'; this.value=this.value+' [disabled]'">	
